...
#read_signed_dec_multi_hex $1 12 200 201 100
...
function fast_read_rx1(){
	#port: $1
	local port=$1
	local start_register=01
	local end_register=250
	local page=27
	
	declare -A rx_power_values
	echo "Registers $start_register To $end_register (Page $page)..."
	
	for (( reg=$start_register; reg<=end_register; reg+=2 )); do
		local rx_mw=$(read_signed_dec_multi_hex $port $page $reg $((reg+1)) 10000)
	#echo "$rx_mw"
	#local rx_mw=0.0121
		
		local result=$(awk -v rx="$rx_mw" 'BEGIN {print 10 * log(rx) / log(10)}')
		rx_power_values["$reg-$((reg+1))"]="$result"
		echo "Register $reg-$((reg+1)): RX = $result dBm"
	done

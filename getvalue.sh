...
function read_input(){
	#port: $1
	local rx_mw=$(read_signed_dec_multi_hex $1 00 74 75 10000)
	#echo "$rx_mw"
	#local rx_mw=0.0121
...
function read_output(){
	#port: $1
	local rx_mw=$(read_signed_dec_multi_hex $1 00 78 79 10000)
	#echo "$rx_mw"
	#local rx_mw=0.0121
	result=$(awk -v rx="$rx_mw" 'BEGIN {print 10 * log(rx) / log(10)}')
	echo "$result"
...
function read_gain(){
	#port: $1
	local rx_mw=$(read_signed_dec_multi_hex $1 00 80 81 10000)
	#echo "$rx_mw"
	#local rx_mw=0.0121
	result=$(awk -v rx="$rx_mw" 'BEGIN {print 10 * log(rx) / log(10)}')
	echo "$result"
